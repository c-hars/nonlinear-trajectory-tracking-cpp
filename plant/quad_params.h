...
struct QuadParams {
    ...
    VecNU  nominal_omegas = VecNU::Zero();  // hover rotor speeds      [rad/s]
    VecNU  max_du         = VecNU::Zero();  // upper clamp on delta_u  [rad/s]
    ...
    VecNU  enabled        = VecNU::Ones();  // coefficients in [0,1] (1 == full health)
